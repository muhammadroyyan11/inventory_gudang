...
{
	var $table = 'kategori';
	var $column_order = array(null, 'kode', 'name', null); //set column field database for datatable orderable
	...
	public function store()
	{
		...
		if ($cek->num_rows() > 0) {
			echo json_encode(array('success' => false, 'message' => 'Nama kategori sudah ada'));
			exit;
		}

		$kode = $this->input->post('kode');
		if ($kode == '') {
			$kode = generate_code('kategori', 'kode', 'KTG-');
		}
		$data = array(
