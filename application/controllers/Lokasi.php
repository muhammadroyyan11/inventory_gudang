...
{
	var $table = 'lokasi';
	var $column_order = array('kode', 'name','hp','alamat', null); //set column field database for datatable orderable
	var $column_search = array('kode', 'name','hp','alamat'); //set column field database for datatable searchable just firstname , lastname , address are searchable
	...
	public function get_all(){
		if ($this->session->userdata('level') == 3) {
			...
			$this->db->join('user_lokasi', 'user_lokasi.lokasi_id=lokasi.id','left');
			$this->db->where('user_lokasi.user_id', $this->session->userdata('id'));
		}
	...
	public function store()
	{
		...
		if ($cek->num_rows() > 0) {
			echo json_encode(array('success' => false, 'message' => 'Nama lokasi sudah ada'));
			exit;
		}
		$kode = $this->input->post('kode');
		if ($kode == '') {
			$kode = generate_code('lokasi', 'kode', 'LKS-');
		}
		$data = array(
