...
class User extends CI_Controller
{
	...
	{
		is_login();
		$data['user'] = $this->global->get_by_id($this->session->userdata('id'), $this->table);
		$this->load->view('template/header');
	...
	public function ajax_table()
	{
		...
		foreach ($list as $r) {
			...
			$akses = '';
			if ($r->level == 3) {
				$akses = '<button data-id="' . $r->id . '" class="btn btn-info btn-sm akses">Akses Lokasi</button>';
			}
			$row[] = $akses . ' <button class="btn btn-warning btn-sm edit" title="Edit" data-id="' . $r->id . '"><i class="bi bi-pencil-square"></i> </button> <button class="btn btn-sm btn-danger delete" title="Hapus" data-id="' . $r->id . '"><i class="bi bi-trash"></i> </button>';
			$response[] = $row;
		}
	...
	public function proses_login()
	{
		...
		if ($user) {
			if (password_verify($password, $user->password)) {
				...
				$myLokasi = $this->global->get_where(array('user_id'=>$user->id), 'user_lokasi')->row_array();
				$gudang = str_split($myLokasi['lokasi_id']);
